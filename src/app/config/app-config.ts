// Intro Component

export const ProfileImage = {
  src: 'assets/images/profile.jpg',
  alt: 'Profile Image',
};

export const IntroData = {
  name: 'Sankarkumar Shanmugagani',
  headline:
    'Cloud/DevOps Architect | Python Software Engineer',
  mail: 'kumaran.online@live.com',
  copiedText: 'Copied',
};

// Profile Component

export const ProfileData = {
  heading: 'Professional Profile',
  data: [
    'Professional level Cloud/DevOps architect with Software Engg. - 19 years of IT experience - DataCenter => Cloud Migration, DevOps, SRE',
    'Multi-skilled in various stacks - AWS, Azure, Kubernetes, Terraform, Packer, Vault, Gitlab, Cloud Security, Networking, HPC, ITIL, Scrum Methodologies, SDLC',
    'Have a very good experience in Identity Services & Backend Python development',
    'Seeking a challenging role in a dynamic organization where I can apply my skills and grow along with the company & team',
  ],
};

// Skills Component

export const SkillsData = {
  heading: 'Skills & Technologies',
  data: [
    'Proficient in migrating applications and infrastructure from on-premises data center to cloud',
    'Proficient in writing Terraform scripts, modules for managing resources across different platforms - AWS, Azure, Kubernetes, NewRelic, VMWare',
    'Proficient in automating configuration management across 100s of servers both on-premises & cloud',
    'Proficient in automating patch management across 100s of servers in AWS using Systems Manager (SSM)',
    'Proficient in building backend services & APIs using Python',
    'Proficient in writing Dockerfiles & publishing images to JFrog Artifactory',    
    'Proficient in building K8s clusters from scratch and configure RBAC for securing them',
    'Proficient in setting up Azure Entra ID & configuring PIM workflows',
    'Proficient in setting up Okta tenants using Terraform & securing them',
    'Proficient in setting up Saviynt for Governance and Audits of the privilaged application/infra roles',    
    'Knowledge of operational management of Slurm On-Premises clusters (HPC)',
    'Proficient in automating repetitive maintenance tasks using Powershell, Bash & Python',    
    'Basic Knowledge of Machine Learning concepts and techniques.',
    'Basic Knowledge of Snowflake and data pipelines',
    'Familiarity with database management systems like MySQL and Oracle',
    'Familiarity with Agile methodologies and Scrum framework.',
  ],
};

// Work Component

export const WorkData = {
  heading: 'Work Experience',
  data: [
    {
      role: 'Architect - Cloud, Identity & Platform Services',
      date: 'May, 2019 - Present',
      company: 'Acadian Asset Management',
      location: 'Boston, MA USA',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },
    {
      role: 'Senior DevOps Engineer',
      date: 'May, 2018 - Apr, 2019',
      company: 'Wellington Management (C)',
      location: 'Boston, MA USA',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },
    {
      role: 'Senior Lead Engineer',
      date: 'Oct, 2016 - Apr, 2018',
      company: 'HID Global',
      location: 'Chennai, TN, India',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },        
    {
      role: 'Automation Consultant',
      date: 'Aug, 2016 - Oct, 2016',
      company: 'Hewlett Packard Enterprise',
      location: 'Bengaluru, KA, India',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },        
    {
      role: 'Architect',
      date: 'Sep, 2011 - Aug, 2016',
      company: 'Verizon',
      location: 'Chennai, TN, India',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },               
    {
      role: 'IT Analyst',
      date: 'Apr, 2005 - Sep, 2011',
      company: 'Tata Consultancy Services',
      location: 'Chennai, TN, India',
      works: [
        'Worked as an Angular developer on the GE HealthCare project entitled Digitized Problem-Solving Report (dPSR).',
        'Implemented a single-page application (SPA) using Angular 10, TypeScript, JavaScript, and SASS.',
        "Integrated various libraries such as Intro.js, NgBootstrap, Chart.js, Ng-Multiselect Dropdown, Ng2-Charts, etc., to enhance the application's functionality.",
        'Implemented the application tour functionality using Intro.js, resulting in improved usability.',
        'Collaborated with the development team to design and implement software solutions to meet business requirements.',
        'Conducted testing, troubleshooting, and debugging of code issues to ensure high-quality deliverables.',
      ],
    },             
  ],
};

// Education Component

export const EducationData = {
  heading: 'Education',
  data: [
    {
      degree: 'B Tech in Electronics & Instrumentation Engineering',
      date: 'Jun 2000 - May 2004',
      college: 'Shanmugha College of Engineering',
      university: 'Bharathidasan University',
      location: 'Thanjavur, TamilNadu, India',
      points: [
        'Aggregate: 79%'
      ],
    },
  ],
};

// Footer Component
export const FooterData = {
  info: 'Sankarkumar Shanmugagani - Digital Resume',
  version: 'v1.0',
};
